...
export class MyError {
	...
	private static readonly filePaths: string[] = [];
	 ...
	 * @param filePaths 文件
	 */
	public static UpdateErrorFilePaths(filePaths: string[]): void {
		// @ts-ignore
		MyError.filePaths = filePaths;
	}
	//#endregion 更新错误信息包含的文件
	...
	SetPosition(option?: { fileIndex?: number, lineNumber?: number, startPosition?: number, length?: number }) {
		if (option?.fileIndex != undefined)
			this.filePath = MyError.filePaths[option.fileIndex];
