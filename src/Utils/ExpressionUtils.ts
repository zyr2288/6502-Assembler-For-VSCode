...
export class ExpressionUtils {
	...
	static CheckExpressionCurrect(
		...
		try {
			...
			err.SetPosition({ filePath: option.globalVar.filePaths[option.fileIndex], lineNumber: option.lineNumber });
		...
		for (let i = 0; i < exps.length; i++) {
			switch (exps[i].partType) {
				case ExpressionPartType.Mark:
					...
					let replace = option.replaceMark?.filter((value) => {
						return value.text.text == word.text;
					});
					if (replace?.length != 0) {
						continue;
					}
					...
					if (!mark || mark.scope == MarkScope.Macro) {
						...
						error.SetPosition({
							filePath: option.globalVar.filePaths[option.fileIndex], lineNumber: option.lineNumber,
